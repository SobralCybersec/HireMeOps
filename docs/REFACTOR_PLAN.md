...
- [x] **Prettier — clean.** Ran `bun run format` across `src` (cleared the pre-existing drift); `format:check` now GREEN and wired into the CI `frontend` job (typecheck · lint · format · test) so it can't drift back.
