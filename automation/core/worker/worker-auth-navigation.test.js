...
function browserWith(page, probePage = page) {
  return {
    pages: vi.fn(() => [page]),
    newPage: vi.fn(async () => probePage),
  };
}
...
describe("login probe navigation lifecycle", () => {
  it("does not navigate an existing LinkedIn page for a single check", async () => {
    ...
    const probePage = pageAt("https://www.linkedin.com/checkpoint/challenge/");
    const browser = browserWith(page, probePage);
    ...
    expect(probePage.goto).toHaveBeenCalledWith("https://www.linkedin.com/feed/", {
      waitUntil: "commit",
      timeout: 30_000,
    });
    expect(probePage.close).toHaveBeenCalled();
  });
  ...
  it("classifies open portal pages without resetting them during all-site checks", async () => {
    ...
    const probePage = pageAt("https://www.linkedin.com/feed/", { authenticatedMarkers: 1 });
    const browser = browserWith(page, probePage);
    sessions.set("handle-1", { user_data_dir: "profile-1", browser, page });
    ...
    expect(page.goto).not.toHaveBeenCalled();
    expect(probePage.goto).toHaveBeenCalled();
    expect(probePage.close).toHaveBeenCalled();
  });

  it("does not treat an arbitrary LinkedIn page as authenticated", async () => {
    const page = pageAt("https://www.linkedin.com/jobs/search/");
    const probePage = pageAt("https://www.linkedin.com/jobs/search/");
    const browser = browserWith(page, probePage);
    sessions.set("handle-1", { user_data_dir: "profile-1", browser, page });
